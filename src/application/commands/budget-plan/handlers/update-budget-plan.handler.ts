...
export class UpdateBudgetPlanHandler {
    ...
    async execute(cmd: UpdateBudgetPlanCommand): Promise<BudgetPlan> {
        ...
        if (!budgetPlan) {
            throw new UseCaseException('Budget plan not found', 'UpdateBudgetPlanHandler');
        }

        budgetPlan.updateBudgetDetails(cmd.budgetType, cmd.allocatedAmount, cmd.fiscalYear);
