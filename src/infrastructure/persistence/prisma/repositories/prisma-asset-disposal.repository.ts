...
export class PrismaAssetDisposalRepository implements IAssetDisposalRepository {
    ...
    async findByAccountingEntry(accountingEntryId: string): Promise<AssetDisposal | null> {
    ...
    async saveMany(disposals: AssetDisposal[]): Promise<void> {
        const data = disposals.map(d => AssetDisposalMapper.toPersistence(d).data);
    ...
    async getDisposalSummary(
        ...
        Object.values(AssetDisposalType).forEach(t => result.byType[t] = { count: 0, value: 0 });
