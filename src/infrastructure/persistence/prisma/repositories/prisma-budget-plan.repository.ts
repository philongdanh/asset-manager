...
import { BudgetPlanMapper } from 'src/infrastructure/mappers';
...
export class PrismaBudgetPlanRepository implements IBudgetPlanRepository {
    ...
    async findAll(
        ...
        const [raws, total] = await Promise.all([
            this.prisma.budgetPlan.findMany({
                ...
                include: options?.includeDepartmentInfo ? { department: true } : undefined,
            }),
    ...
    async getRemainingBudget(
        ...
        // Aggregation of all budget plans for this criteria
        ...
        for (const plan of plans) {
            totalRemaining += plan.allocatedAmount.toNumber() - plan.spentAmount.toNumber();
        }
    ...
    async getBudgetSummary(
        ...
        for (const plan of plans) {
            ...
            if (!result.byDepartment[plan.departmentId]) {
                result.byDepartment[plan.departmentId] = { allocated: 0, spent: 0, remaining: 0, utilizationRate: 0 };
            }
            const dept = result.byDepartment[plan.departmentId];
            dept.allocated += allocated;
            dept.spent += spent;
            dept.remaining += remaining;
            dept.utilizationRate = dept.allocated > 0 ? (dept.spent / dept.allocated) * 100 : 0;

            // By Type
            const type = plan.budgetType as BudgetType;
            if (!result.byType[type]) {
                result.byType[type] = { allocated: 0, spent: 0, remaining: 0, planCount: 0 };
            }
    ...
    async findOverBudgetPlans(
        ...
        // Logic: spent > allocated (or threshold condition if we had one)
        // Prisma doesn't easily support "where column A > column B".
        // Retrieve all and filter in app.
        const raws = await this.prisma.budgetPlan.findMany({
            where: { organizationId },
        });
    ...
    async getBudgetUtilizationReport(
        ...
        const departmentUtilization = Object.keys(summary.byDepartment).map(deptId => {
            const data = summary.byDepartment[deptId];
            return {
                departmentId: deptId,
                departmentName: 'Unknown', // Ideally fetch department name, but here return ID or doing extra query
                ...data
            };
        });

        // If we want department names, we need to query departments.
        // Optimization: fetch departments where id in keys.
        ...
        const overallUtilization = summary.totalAllocated > 0 ? (summary.totalSpent / summary.totalAllocated) * 100 : 0;

        return {
            overallUtilization,
            departmentUtilization,
            monthlySpending: [], // Placeholder as we don't have transaction history in BudgetPlan
        };
    }
    ...
    async getBudgetForecast(
        ...
        // Simple linear projection based on current utilization
        ...
        // Assume spent over X months passed in year?
        ...
        return {
            ...
            projectedRemaining,
            willExceedBudget,
            estimatedExceedDate: willExceedBudget ? new Date() : undefined // Placeholder logic
        };
    }
