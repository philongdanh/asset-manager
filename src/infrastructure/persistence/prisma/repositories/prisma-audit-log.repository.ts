...
import { AuditLogMapper } from 'src/infrastructure/mappers';
...
export class PrismaAuditLogRepository implements IAuditLogRepository {
    ...
    async findAll(
        ...
        if (options?.action) {
            where.action = Array.isArray(options.action) ? { in: options.action as string[] } : options.action as string;
        }
        if (options?.entityType) {
            where.entityType = Array.isArray(options.entityType) ? { in: options.entityType as string[] } : options.entityType as string;
        }
        if (options?.startDate || options?.endDate) {
            where.actionTime = {};
            if (options.startDate) where.actionTime.gte = options.startDate;
    ...
    async findByAction(action: AuditAction): Promise<AuditLog[]> {
        const raws = await this.prisma.auditLog.findMany({ where: { action: action as string } });
        return raws.map(AuditLogMapper.toDomain);
    }

    async findByEntityType(entityType: EntityType): Promise<AuditLog[]> {
        const raws = await this.prisma.auditLog.findMany({ where: { entityType: entityType as string } });
        return raws.map(AuditLogMapper.toDomain);
    }
    ...
    async getAuditSummary(
        ...
        return {
            ...
            byEntityType,
            byUser,
            dailyCounts: Object.entries(dailyCountsMap).map(([date, count]) => ({ date, count })),
        };
    }

    async getEntityAuditTrail(entityType: EntityType, entityId: string): Promise<AuditLog[]> {
        return this.findByEntity(entityType, entityId);
    }

    async getUserActivityReport(userId: string, startDate: Date, endDate: Date): Promise<any> {
        const logs = await this.prisma.auditLog.findMany({
        ...
        for (const log of logs) {
            ...
            entitiesModified[log.entityType] = (entitiesModified[log.entityType] || 0) + 1;
        ...
        return {
            ...
            actionsByType,
            entitiesModified,
            peakActivityHours: Object.entries(hoursMap).map(([h, c]) => ({ hour: +h, count: c })),
            recentActions: logs.slice(0, 50).map(AuditLogMapper.toDomain),
        };
    }

    async findRecentLogs(organizationId: string, limit: number): Promise<AuditLog[]> {
        const raws = await this.prisma.auditLog.findMany({
    ...
    async findSuspiciousActivities(organizationId: string, options?: any): Promise<any> {
    ...
    async exportAuditLogs(organizationId: string, options: any): Promise<string> {
        return '';
    }

    async logSearch(organizationId: string, criteria: any): Promise<AuditLog[]> {
        return [];
    }

    async getLogStatistics(organizationId: string, period: any): Promise<any> {
        return {};
    }
}
