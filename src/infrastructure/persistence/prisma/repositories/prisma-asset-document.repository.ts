...
import { AssetDocumentMapper } from 'src/infrastructure/mappers/asset-document.mapper';
...
export class PrismaAssetDocumentRepository implements IAssetDocumentRepository {
    ...
    async findByDocumentType(organizationId: string, documentType: AssetDocumentType): Promise<AssetDocument[]> {
        const raws = await this.prisma.assetDocument.findMany({
    ...
    async findAll(
        ...
        if (options?.uploadedByUserId) where.uploadedByUserId = options.uploadedByUserId;
    ...
    async existsByFilePath(filePath: string): Promise<boolean> {
        const count = await this.prisma.assetDocument.count({ where: { filePath } });
        return count > 0;
    }
    ...
    async saveMany(documents: AssetDocument[]): Promise<void> {
        const data = documents.map(d => AssetDocumentMapper.toPersistence(d).data);
    ...
    async getDocumentsSummary(organizationId: string): Promise<{
        ...
        recentUploads: Array<{ documentId: string; documentName: string; assetId: string; uploadDate: Date; uploadedBy: string }>;
    }> {
        const documents = await this.prisma.assetDocument.findMany({
            where: { organizationId },
            select: { id: true, documentType: true, assetId: true, documentName: true, uploadDate: true, uploadedByUserId: true },
            orderBy: { uploadDate: 'desc' },
        });
        ...
        const result = {
            ...
            recentUploads: [] as Array<{ documentId: string; documentName: string; assetId: string; uploadDate: Date; uploadedBy: string }>,
        };
    ...
    async findDocumentsByFileType(organizationId: string, fileType: string): Promise<AssetDocument[]> {
        const raws = await this.prisma.assetDocument.findMany({
    ...
    async findRecentDocuments(organizationId: string, limit: number): Promise<AssetDocument[]> {
        const raws = await this.prisma.assetDocument.findMany({
            where: { organizationId },
            orderBy: { uploadDate: 'desc' },
            take: limit,
    ...
    async getDocumentTypeDistribution(organizationId: string): Promise<Record<AssetDocumentType, number>> {
        const documents = await this.prisma.assetDocument.findMany({
            where: { organizationId },
    ...
    async getUploadActivity(organizationId: string, startDate: Date, endDate: Date): Promise<Array<{ date: string; count: number }>> {
        const raws = await this.prisma.$queryRaw<any[]>`
            ...
            ORDER BY date ASC
        `;
        return raws.map(r => ({ date: new Date(r.date).toISOString().split('T')[0], count: Number(r.count) }));
    }

    async findByAssetIds(assetIds: string[]): Promise<Record<string, AssetDocument[]>> {
        const raws = await this.prisma.assetDocument.findMany({
