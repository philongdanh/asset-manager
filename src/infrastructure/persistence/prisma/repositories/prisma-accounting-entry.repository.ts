...
import { AccountingEntryMapper } from 'src/infrastructure/mappers';
...
export class PrismaAccountingEntryRepository implements IAccountingEntryRepository {
    ...
    async findByOrganization(organizationId: string): Promise<AccountingEntry[]> {
        ...
        return raws.map(AccountingEntryMapper.toDomain);
    }
    ...
    async findAll(
        ...
        return {
            data: raws.map(AccountingEntryMapper.toDomain),
            total,
        };
    }
    ...
    async deleteMany(ids: string[]): Promise<void> {
        await this.prisma.accountingEntry.deleteMany({ where: { id: { in: ids } } });
    }
    ...
    async deleteByReference(referenceType: ReferenceType, referenceId: string): Promise<void> {
        await this.prisma.accountingEntry.deleteMany({ where: { referenceType, referenceId } });
    }
    ...
    async getFinancialSummary(
        ...
        const result = {
            ...
            totalAmount: 0,
            byEntryType: {} as Record<AccountingEntryType, { count: number; amount: number }>,
        ...
        for (const raw of raws) {
            ...
            if (!result.byEntryType[type]) result.byEntryType[type] = { count: 0, amount: 0 };
            result.byEntryType[type].count++;
            result.byEntryType[type].amount += amount;

            // By Asset
            if (raw.assetId) {
                if (!result.byAsset[raw.assetId]) result.byAsset[raw.assetId] = { count: 0, amount: 0 };
            ...
            if (!monthMap.has(monthKey)) monthMap.set(monthKey, { count: 0, amount: 0 });
    ...
    > {
        ...
        for (let i = 0; i < 12; i++) {
            summaryMap.set(i, { totalAmount: 0, entryCount: 0, byType: {} as Record<AccountingEntryType, number> });
        }
    ...
    async getAssetFinancialHistory(assetId: string): Promise<{
        ...
        const raws = await this.prisma.accountingEntry.findMany({ where: { assetId } });
