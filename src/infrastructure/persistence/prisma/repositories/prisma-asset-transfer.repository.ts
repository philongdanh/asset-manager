...
import { AssetTransferMapper } from 'src/infrastructure/mappers/asset-transfer.mapper';
...
export class PrismaAssetTransferRepository implements IAssetTransferRepository {
    ...
    async findAll(
        ...
        if (options?.fromDepartmentId) where.fromDepartmentId = options.fromDepartmentId;
        ...
        const [data, total] = await Promise.all([
            this.prisma.assetTransfer.findMany({
                ...
                skip: options?.offset,
                orderBy: { transferDate: 'desc' },
                // include: options?.includeAssetInfo ? { asset: true } : undefined, // Check if we need to map Asset entity or stay with AssetTransfer
            }),
    ...
    async saveMany(transfers: AssetTransfer[]): Promise<void> {
        const data = transfers.map(t => AssetTransferMapper.toPersistence(t).data);
    ...
    async getTransfersSummary(
        ...
        const result = {
            ...
            byDepartment: {} as Record<string, { incoming: number; outgoing: number }>,
        ...
        for (const t of transfers) {
            ...
            if (t.fromDepartmentId) {
                if (!result.byDepartment[t.fromDepartmentId]) result.byDepartment[t.fromDepartmentId] = { incoming: 0, outgoing: 0 };
                result.byDepartment[t.fromDepartmentId].outgoing++;
            }
            if (t.toDepartmentId) {
                if (!result.byDepartment[t.toDepartmentId]) result.byDepartment[t.toDepartmentId] = { incoming: 0, outgoing: 0 };
                result.byDepartment[t.toDepartmentId].incoming++;
            }

            // Users
            if (t.fromUserId) {
                if (!result.byUser[t.fromUserId]) result.byUser[t.fromUserId] = { incoming: 0, outgoing: 0 };
                result.byUser[t.fromUserId].outgoing++;
            }
            if (t.toUserId) {
                if (!result.byUser[t.toUserId]) result.byUser[t.toUserId] = { incoming: 0, outgoing: 0 };
                result.byUser[t.toUserId].incoming++;
            }
        }

        // @ts-ignore: Fix type 'any' is not assignable to type 'Record<AssetTransferType, number>'
    ...
    > {
        // Basic aggregation using findMany then process in memory, or use groupBy
        // Prisma groupBy for Date field might be tricky without raw query.
        // Let's fetch and process for simplicity if data volume isn't huge, or use raw query.
