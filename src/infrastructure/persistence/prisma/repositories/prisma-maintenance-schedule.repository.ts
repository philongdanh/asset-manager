...
import { MaintenanceScheduleMapper } from 'src/infrastructure/mappers/maintenance-schedule.mapper';
...
export class PrismaMaintenanceScheduleRepository implements IMaintenanceScheduleRepository {
    ...
    async findUpcoming(organizationId: string, daysAhead: number): Promise<MaintenanceSchedule[]> {
        const now = new Date();
    ...
    async findOverdue(organizationId: string): Promise<MaintenanceSchedule[]> {
        ...
        const raws = await this.prisma.maintenanceSchedule.findMany({
            where: {
                ...
                status: { in: [MaintenanceStatus.SCHEDULED, MaintenanceStatus.APPROVED] },
            },
            orderBy: { scheduledDate: 'asc' },
        });
        return raws.map(r => MaintenanceScheduleMapper.toDomain(r));
    ...
    async findByStatus(organizationId: string, status: MaintenanceStatus): Promise<MaintenanceSchedule[]> {
        const raws = await this.prisma.maintenanceSchedule.findMany({
    ...
    async findByType(organizationId: string, maintenanceType: MaintenanceType): Promise<MaintenanceSchedule[]> {
        const raws = await this.prisma.maintenanceSchedule.findMany({
    ...
    async findByPerformer(performedByUserId: string): Promise<MaintenanceSchedule[]> {
        const raws = await this.prisma.maintenanceSchedule.findMany({
    ...
    async findCompletedInPeriod(organizationId: string, startDate: Date, endDate: Date): Promise<MaintenanceSchedule[]> {
        const raws = await this.prisma.maintenanceSchedule.findMany({
            where: {
                organizationId,
    ...
    async hasActiveMaintenance(assetId: string): Promise<boolean> {
        const count = await this.prisma.maintenanceSchedule.count({
            where: {
                assetId,
                status: { in: [MaintenanceStatus.SCHEDULED, MaintenanceStatus.IN_PROGRESS, MaintenanceStatus.APPROVED] },
            },
        });
        return count > 0;
    }
    ...
    async existsById(maintenanceId: string): Promise<boolean> {
        const count = await this.prisma.maintenanceSchedule.count({ where: { id: maintenanceId } });
        return count > 0;
    }
    ...
    async saveMany(maintenances: MaintenanceSchedule[]): Promise<void> {
        const data = maintenances.map(m => MaintenanceScheduleMapper.toPersistence(m).data);
    ...
    async delete(maintenanceId: string): Promise<void> {
        await this.prisma.maintenanceSchedule.delete({ where: { id: maintenanceId } });
    }

    async deleteMany(maintenanceIds: string[]): Promise<void> {
        await this.prisma.maintenanceSchedule.deleteMany({ where: { id: { in: maintenanceIds } } });
    }
    ...
    async getMaintenanceSummary(organizationId: string): Promise<{
        ...
        const maintenances = await this.prisma.maintenanceSchedule.findMany({
            ...
            select: { status: true, maintenanceType: true, estimatedCost: true, actualCost: true, scheduledDate: true },
        });
        ...
        for (const m of maintenances) {
            ...
            result.totalEstimatedCost += m.estimatedCost ? Number(m.estimatedCost) : 0;
            ...
            if (m.scheduledDate < now && [MaintenanceStatus.SCHEDULED, MaintenanceStatus.APPROVED].includes(m.status as MaintenanceStatus)) {
                result.overdueCount++;
            }
        }
    ...
    async getAssetMaintenanceHistory(assetId: string): Promise<MaintenanceSchedule[]> {
        return this.findByAssetId(assetId);
    }

    async getMaintenanceCostAnalysis(organizationId: string, startDate: Date, endDate: Date): Promise<{
        totalCost: number;
        byType: Record<MaintenanceType, number>;
        ...
        const maintenances = await this.prisma.maintenanceSchedule.findMany({
            ...
            select: { maintenanceType: true, assetId: true, actualCost: true, actualDate: true },
        });
        ...
        const result = {
            ...
            monthlyTrends: [] as Array<{ month: string; cost: number; count: number }>,
        };
        ...
        for (const m of maintenances) {
            ...
            if (m.actualDate) {
                ...
                monthlyMap.set(monthKey, { cost: existing.cost + cost, count: existing.count + 1 });
            }
        }
    ...
    async findMaintenanceDueForReview(organizationId: string, daysThreshold: number): Promise<MaintenanceSchedule[]> {
        return this.findUpcoming(organizationId, daysThreshold);
    }

    async getMaintenanceFrequencyReport(organizationId: string, period: 'month' | 'quarter' | 'year'): Promise<{
        period: string;
    ...
    async getMaintenanceBacklog(organizationId: string): Promise<{
        ...
        return {
            ...
            pendingApproval: summary.byStatus[MaintenanceStatus.PENDING_APPROVAL] || 0,
            overdue: summary.overdueCount,
        };
    }

    async findHighCostMaintenances(organizationId: string, threshold: number): Promise<MaintenanceSchedule[]> {
        const raws = await this.prisma.maintenanceSchedule.findMany({
            where: {
                organizationId,
    ...
    async getTechnicianPerformanceReport(organizationId: string, startDate: Date, endDate: Date): Promise<Array<{
        userId: string;
    ...
    async findRecurringIssues(organizationId: string, lookbackDays: number): Promise<Array<{
        assetId: string;
        issuePattern: string;
    ...
    async exportMaintenanceReport(organizationId: string, options: {
        startDate: Date;
        endDate: Date;
        format: 'CSV' | 'JSON' | 'PDF';
    }): Promise<string> {
