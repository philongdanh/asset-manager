...
import { AssetDepreciationMapper } from 'src/infrastructure/mappers/asset-depreciation.mapper';
...
export class PrismaAssetDepreciationRepository implements IAssetDepreciationRepository {
    ...
    async findById(id: string): Promise<AssetDepreciation | null> {
        const raw = await this.prisma.assetDepreciation.findUnique({ where: { id } });
    ...
    async findByOrganization(organizationId: string): Promise<AssetDepreciation[]> {
        const raws = await this.prisma.assetDepreciation.findMany({
    ...
    async findByAccountingEntry(accountingEntryId: string): Promise<AssetDepreciation | null> {
        const raw = await this.prisma.assetDepreciation.findFirst({ where: { accountingEntryId } });
        return raw ? AssetDepreciationMapper.toDomain(raw) : null;
    }

    async findByMethod(organizationId: string, method: DepreciationMethod): Promise<AssetDepreciation[]> {
        const raws = await this.prisma.assetDepreciation.findMany({
    ...
    async findAll(
        ...
        if (options?.maxValue !== undefined) {
            where.depreciationValue = { ...(where.depreciationValue as object || {}), lte: options.maxValue };
        }
    ...
    async findLatestByAssetId(assetId: string): Promise<AssetDepreciation | null> {
    ...
    async findByDateRange(assetId: string, startDate: Date, endDate: Date): Promise<AssetDepreciation[]> {
        const raws = await this.prisma.assetDepreciation.findMany({
            where: {
                assetId,
    ...
    async saveMany(depreciations: AssetDepreciation[]): Promise<void> {
        const data = depreciations.map(d => AssetDepreciationMapper.toPersistence(d).data);
    ...
    async deleteMany(ids: string[]): Promise<void> {
        await this.prisma.assetDepreciation.deleteMany({ where: { id: { in: ids } } });
    }
    ...
    async deleteByAccountingEntry(accountingEntryId: string): Promise<void> {
        await this.prisma.assetDepreciation.deleteMany({ where: { accountingEntryId } });
    }

    async getDepreciationSummary(organizationId: string, fiscalYear?: number): Promise<{
        ...
        const depreciations = await this.prisma.assetDepreciation.findMany({
            where,
            select: { method: true, depreciationValue: true, assetId: true, depreciationDate: true },
        });

        const result = {
            totalDepreciation: 0,
            totalAssets: new Set<string>(),
            byMethod: {} as Record<DepreciationMethod, { count: number; value: number }>,
            byMonth: [] as Array<{ month: string; count: number; totalValue: number }>,
        };

        Object.values(DepreciationMethod).forEach(m => result.byMethod[m] = { count: 0, value: 0 });
        ...
        for (const d of depreciations) {
            ...
            monthlyMap.set(monthKey, { count: existing.count + 1, totalValue: existing.totalValue + value });
        }
    ...
    async getAssetDepreciationHistory(assetId: string): Promise<{
        ...
        yearlyBreakdown: Array<{ year: number; depreciation: number; accumulated: number; remaining: number }>;
    }> {
        const depreciations = await this.findByAssetId(assetId);
        const latest = depreciations[0];

        const yearlyMap = new Map<number, { depreciation: number; accumulated: number; remaining: number }>();
        for (const d of depreciations) {
            const year = d.depreciationDate.getFullYear();
            yearlyMap.set(year, {
                depreciation: (yearlyMap.get(year)?.depreciation || 0) + d.depreciationValue,
        ...
        return {
            totalDepreciation: depreciations.reduce((sum, d) => sum + d.depreciationValue, 0),
    ...
    async findDepreciationsWithoutAccountingEntry(organizationId: string): Promise<AssetDepreciation[]> {
        const raws = await this.prisma.assetDepreciation.findMany({
    ...
    async getMonthlyDepreciationReport(organizationId: string, year: number, month: number): Promise<{
        totalDepreciation: number;
        byAssetCategory: Record<string, number>;
        byDepartment: Record<string, number>;
        topDepreciatingAssets: Array<{ assetId: string; assetName: string; depreciationValue: number }>;
    }> {
    ...
    async getDepreciationsByAssetIds(assetIds: string[]): Promise<Record<string, AssetDepreciation[]>> {
        const raws = await this.prisma.assetDepreciation.findMany({
