...
import { RecordDepreciationCommand } from 'src/application/commands/asset-depreciation';
import { RecordDepreciationHandler } from 'src/application/commands/asset-depreciation/handlers';
import { GetAssetDepreciationsQuery, GetAssetDepreciationByIdQuery } from 'src/application/queries/asset-depreciation';
import {
...
export class AssetDepreciationController {
    ...
    async getList(
        ...
        const q = new GetAssetDepreciationsQuery(organizationId || '', { assetId, limit, offset });
    ...
    async getDetails(
        ...
        const query = new GetAssetDepreciationByIdQuery(id);
        const result = await this.getDetailsHandler.execute(query);
        return this.toResponse(result);
    }

    private toResponse(entity: any): AssetDepreciationResponse {
